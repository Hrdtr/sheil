...
A practical, opinionated runbook for safely updating dependencies in this project.
...
cargo update --manifest-path tauri/Cargo.toml

# For major version bumps, manually edit Cargo.toml version strings,
# then run:
cargo build --manifest-path tauri/Cargo.toml
```
...
nub install --save-dev svelte@latest @sveltejs/vite-plugin-svelte@latest svelte-check@latest
...
For major version bumps, edit `tauri/Cargo.toml` manually:
...
cargo outdated --manifest-path tauri/Cargo.toml

# Preview safe lockfile updates without applying
cargo update --manifest-path tauri/Cargo.toml --dry-run
```
...
| --------------------------------- | ---------------------------------------------- | -------------------------------------------------------------------------- |
| **Weekly**                        | `nub update -i` + `cargo update` (semver-safe) | Phase 1 only — should always pass                                          |
| **Monthly**                       | `nub update -i --latest` (all latest)          | Phase 1, fallback to Phase 2                                               |
...
```bash
# ── Diagnose ──
nub outdated                                                    # Frontend
cargo outdated --manifest-path tauri/Cargo.toml                 # Rust

# ── Update (optimistic) ──
nub update -i && nub install                                    # Frontend (all latest)
cargo update --manifest-path tauri/Cargo.toml                   # Rust (semver-safe)

# ── Validate ──
nub run test                                                    # Full pipeline

# ── Individual checks (for debugging) ──
nub run test:check                                              # Types + lint
nub run test:unit                                               # Vitest (92 tests)
nub run test:rust                                               # Cargo test (9 tests)
nub run test:watch                                              # Vitest in watch mode
```
