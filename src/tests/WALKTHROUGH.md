...
| Tool                             | Status | Detail                              |
| -------------------------------- | ------ | ----------------------------------- |
| `nub run test:unit` (Vitest)     | ✅     | 92 tests, 8 suites — ~5s            |
| `nub run test:rust` (cargo test) | ✅     | 9 tests — 0.00s (excl. compilation) |
...
| File                            | Tests | What It Protects                                                                                                       |
| ------------------------------- | ----- | ---------------------------------------------------------------------------------------------------------------------- |
| `tauri/src/commands/default.rs` | 6     | read/write roundtrip, overwrite, empty, unicode, IO error, UTF-8 error                                                 |
| `tauri/src/commands/errors.rs`  | 3     | JSON serialization IO → `{name:"io",message:...}`, UTF8 → `{name:"fromUtf8Error",message:...}`, exact shape (2 fields) |
...
| `tauri/Cargo.toml`                 | `[dev-dependencies] tempfile = "3"`                                                                                 |
...
# Individual commands:
nub run test:unit      # Vitest — 92 tests, ~5s
nub run test:watch     # Vitest watch mode (dev)
nub run test:rust      # cargo test — 9 tests
nub run test:check     # svelte-check + tsc + prettier + eslint
```
